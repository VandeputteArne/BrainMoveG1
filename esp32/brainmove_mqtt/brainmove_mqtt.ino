...
// Verlaagd naar 5 seconden voor snelle detectie van wegvallen!
...
void verbindGeforceerd(); // Nieuwe blokkerende functie
...
// =================== NIEUWE VERBINDINGS LOGICA ===================
void verbindGeforceerd() {
  Serial.println("Verbinding verloren. Apparaat pauzeert...");
  
  // Zet alles uit (visuele feedback dat hij niet werkt)
  rgbLedUit();
  zoemerUit();
  isPolling = false; // Stop met meten
  
  // 1. Zorg dat WiFi er is (Blocking)
  if (WiFi.status() != WL_CONNECTED) {
    ...
    while (WiFi.status() != WL_CONNECTED) {
      delay(500);
      Serial.print(".");
      // Blauw knipperen = Zoeken naar WiFi
  ...
  // 2. Zorg dat MQTT er is (Blocking)
  while (!mqttClient.connected()) {
    // Check of WiFi nog leeft
    ...
    // Last Will message: als hij crasht -> "offline"
    if (mqttClient.connect(clientId.c_str(), topic_status.c_str(), 1, true, "offline")) {
      ...
      // === DE HANDSHAKE ===
      // Stuur "online" + MAC adres zodat Python kan verifiëren
...
void verwerkToF() {
    ...
    if (millis() - laatsteTofPollTijd >= TOF_POLL_INTERVAL_MS) {
        ...
        if (geldigeDetectie && (millis() - laatsteDetectieTijd >= TOF_DETECTIE_AFKOELING_MS)) {
            Serial.print("Detectie: "); Serial.println(afstand);
